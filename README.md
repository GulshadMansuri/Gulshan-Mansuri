# CSE0001: Digital Literacy Project Portfolio
**Student Digital Ambassador Assignment**

## Student Information
* **Name:** Gulshan Nijammudin Shaikh Mansuri
* **Registration No:** 25BCE11141 
* **Branch:** Computer Science and Engineering (First Year) 
* **Course:** CSE0001 - Digital Literacy 

---

## Project Overview
This repository serves as a comprehensive portfolio for the Digital Literacy course at VIT Bhopal University. As a Student Digital Ambassador, I have completed five core modules designed to enhance professional communication, online safety, and technical collaboration.

---

## Repository Structure & Module Summaries

### [Task 1: Digital Literacy Infographic](./task-1-presentation/)
Created a visual guide using **Canva** to help batchmates understand digital tools and safe internet practices
* **Key Topics:** Defining Digital Literacy, Toolkits, and Online Safety.

### [Task 2: Professional Digital Portfolio](./task-2-portfolio/)
Established a professional presence across major platforms to prepare for future internship and academic opportunities.
* **Platforms:** GitHub, LinkedIn, and Kaggle.

### [Task 3: Coding & Collaboration](./task-3-platforms/)
Demonstrated technical proficiency and collaborative skills through coding challenges and data collection.
* **Coding Practice:** Completed beginner Python challenges on **HackerRank**.
* **Collaboration Tool:** Created a 5-question Digital Literacy Quiz using **Google Forms**.
* **Quiz Link:**  https://forms.gle/FpMETtdzkS2htXGP9

### [Task 4: Professional Communication](./task-4-email-etiquette/)
Applied formal email etiquette for academic and professional scenarios.
* **Contents:** Extension request to Professor Reena Jain and an internship inquiry for NVIDIA.
* **Resource:** Social Media "Do's and Don'ts" Checklist.

### [Task 5: Cybercrime Awareness](./task-5-cybercrime/)
Researched real-world threats to provide a safety resource for the student community.
* **Focus:** UPI/Online Payment Fraud Case Study.
* **Resource:** 'Stay Safe Online' 8-point prevention checklist.

---

## Final Project Report
#### The full written report covering all reflections, observations, and detailed task documentation is available here:
 **[View Project Report](./report/Project_Report.pdf)**
---
*Developed as part of the VIT Bhopal University Digital Literacy Program (2026).* 